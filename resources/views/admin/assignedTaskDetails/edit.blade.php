...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.assignedTaskDetail.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.assigned-task-details.update", [$assignedTaskDetail->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('task') ? 'is-invalid' : '' }}" name="task_id" id="task_id">
                    @foreach($tasks as $id => $entry)
                        <option value="{{ $id }}" {{ (old('task_id') ? old('task_id') : $assignedTaskDetail->task->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('task'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('assigned_task') ? 'is-invalid' : '' }}" name="assigned_task_id" id="assigned_task_id" required>
                    @foreach($assigned_tasks as $id => $entry)
                        <option value="{{ $id }}" {{ (old('assigned_task_id') ? old('assigned_task_id') : $assignedTaskDetail->assigned_task->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date') ? 'is-invalid' : '' }}" type="text" name="date" id="date" value="{{ old('date', $assignedTaskDetail->date) }}">
                @if($errors->has('date'))
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('start_time') ? 'is-invalid' : '' }}" type="text" name="start_time" id="start_time" value="{{ old('start_time', $assignedTaskDetail->start_time) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('end_time') ? 'is-invalid' : '' }}" type="text" name="end_time" id="end_time" value="{{ old('end_time', $assignedTaskDetail->end_time) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status_id" id="status_id">
                    @foreach($statuses as $id => $entry)
                        <option value="{{ $id }}" {{ (old('status_id') ? old('status_id') : $assignedTaskDetail->status->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_approved') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_approved" id="is_approved" value="1" {{ $assignedTaskDetail->is_approved || old('is_approved', 0) === 1 ? 'checked' : '' }}>
