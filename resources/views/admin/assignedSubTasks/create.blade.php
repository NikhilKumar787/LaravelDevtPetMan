...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.assignedSubTask.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.assigned-sub-tasks.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            `<div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('sub_task') ? 'is-invalid' : '' }}" name="sub_task_id" id="sub_task_id" required>
                    @foreach($sub_tasks as $id => $entry)
                        <option value="{{ $id }}" {{ old('sub_task_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
