...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.assigned-sub-tasks.update", [$assignedSubTask->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                <label class="required" for="sub_task_id">{{ trans('cruds.assignedSubTask.fields.sub_task') }}</label>
                <select class="form-control select2 {{ $errors->has('sub_task') ? 'is-invalid' : '' }}" name="sub_task_id" id="sub_task_id" required>
                    @foreach($sub_tasks as $id => $entry)
                        <option value="{{ $id }}" {{ (old('sub_task_id') ? old('sub_task_id') : $assignedSubTask->sub_task->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('sub_task'))
                    <span class="text-danger">{{ $errors->first('sub_task') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.assignedSubTask.fields.sub_task_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{{ old('description', $assignedSubTask->description) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('hours_estimation') ? 'is-invalid' : '' }}" type="number" name="hours_estimation" id="hours_estimation" value="{{ old('hours_estimation', $assignedSubTask->hours_estimation) }}" step="1">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_approved') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_approved" id="is_approved" value="1" {{ $assignedSubTask->is_approved || old('is_approved', 0) === 1 ? 'checked' : '' }}>
                    <label class="form-check-label" for="is_approved">{{ trans('cruds.assignedSubTask.fields.is_approved') }}</label>
                </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('target_date') ? 'is-invalid' : '' }}" type="text" name="target_date" id="target_date" value="{{ old('target_date', $assignedSubTask->target_date) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('completed_date') ? 'is-invalid' : '' }}" type="text" name="completed_date" id="completed_date" value="{{ old('completed_date', $assignedSubTask->completed_date) }}">
...
Dropzone.options.requirementDropzone = {
    url: '{{ route('admin.assigned-sub-tasks.storeMedia') }}',
    maxFilesize: 2, // MB
    addRemoveLinks: true,
    headers: {
