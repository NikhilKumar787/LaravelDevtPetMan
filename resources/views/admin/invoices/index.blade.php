...
<div class="card">
    ...
    <div class="px-4 pb-3 bg-white d-flex justify-content-between align-items-center">
        ...
        <div class="">
            <div class="btn-group">
                <a href="{{ route('admin.invoices.create') }}"><button type="button" class="btn btn-success">Create Invoice</button></a>
...
@parent
<script>
    var url = "{{ route('admin.invoices.index') }}";
    @if(request()->query('type')=='paid')
    url = "{{ route('admin.invoices.index',['type'=>'paid']) }}";
    @elseif(request()->query('type')=='unpaid')
    url = "{{ route('admin.invoices.index',['type'=>'unpaid']) }}";
    @elseif(request()->query('type')=='myrequests')
    url = "{{ route('admin.invoices.index',['type'=>'myrequests']) }}";
    @endif
    $(function () {
