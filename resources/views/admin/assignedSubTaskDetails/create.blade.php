...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.assignedSubTaskDetail.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.assigned-sub-task-details.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('assigned_sub_task') ? 'is-invalid' : '' }}" name="assigned_sub_task_id" id="assigned_sub_task_id" required>
                    @foreach($assigned_sub_tasks as $id => $entry)
                        <option value="{{ $id }}" {{ old('assigned_sub_task_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('assigned_sub_task'))
