        ...
        <div class="col-md-12">
            @can('sub_task_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.sub-tasks.create') }}">
                            ...
                            <tbody>
                                @foreach($subTasks as $key => $subTask)
                                    <tr data-entry-id="{{ $subTask->id }}">
                                        ...
                                        <td>
                                            @can('sub_task_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.sub-tasks.show', $subTask->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('sub_task_edit')
                                                <a class="btn btn-link text-theme" href="{{ route('frontend.sub-tasks.edit', $subTask->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('sub_task_delete')
                                                <form action="{{ route('frontend.sub-tasks.destroy', $subTask->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('sub_task_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.sub-tasks.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-SubTask:not(.ajaxTable)').DataTable({ buttons: dtButtons })
