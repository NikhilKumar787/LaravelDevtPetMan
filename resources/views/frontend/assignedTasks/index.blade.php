        ...
        <div class="col-md-12">
            @can('assigned_task_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.assigned-tasks.create') }}">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-AssignedTask">
                            <thead>
                                <tr>
                                    <th>
                                    ...
                                    <tr data-entry-id="{{ $assignedTask->id }}">
                                        ...
                                        <td>
                                            @can('assigned_task_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.assigned-tasks.show', $assignedTask->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('assigned_task_edit')
                                                <a class="btn btn-link text-theme" href="{{ route('frontend.assigned-tasks.edit', $assignedTask->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('assigned_task_delete')
                                                <form action="{{ route('frontend.assigned-tasks.destroy', $assignedTask->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('assigned_task_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.assigned-tasks.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-AssignedTask:not(.ajaxTable)').DataTable({ buttons: dtButtons })
