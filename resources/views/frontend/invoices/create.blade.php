...
<form method="POST" action="{{ route("frontend.invoices.store") }}" enctype="multipart/form-data">
    ...
    <div class="invoice-add">
        ...
        <div class="row mt-2">
            ...
            <div class="col-md-3">
                <div class="form-group">
                    ...
                    <textarea class="form-control" id="ship_address" name="ship_address" placeholder="" rows="3"></textarea>
                </div>
            </div>
                <div class="col-md-2">
                    <div class="form-group">
                ...
                <div class="col-md-2">
                    <div class="form-group">
                        <label for="due_date">Company</label>
                        <input type="text" class="form-control" id="company" name="company" value="{{isset($company->id)? $company->company_name : ''}}">
                    </div>
                    {{-- <div class="form-group">
                        <label for="place_of_supply_id">Place of Supply</label>
                        <select class="form-control select2 {{ $errors->has('place_of_supply_id') ? 'is-invalid' : '' }}" name="place_of_supply_id" id="place_of_supply_id">
        ...
        <select id="amount_tax" name="amount_tax" class="form-control py-0">
        ...
        <tbody>
            <tr>
            {{-- <th scope="row">1</th> --}}
            <td><input type="date" class="form-control" id="service_date" name="service_date[]"></td>
            <td>
                <select class="form-control select2 product_class {{ $errors->has('product') ? 'is-invalid' : '' }}" data-attr="1" id="product_id" name="product_id[]">
            ...
            <td><img src="{{ asset('img/delete.png')}}" alt=""></td>
            </tr>
        </tbody>
        </table>
                            ...
                            <div class="col-md-12">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="form-check ml-3">
                                        <input class="form-check-input" type="checkbox" value="0" id="same_as_bill">
                        ...
                        <div class="tab-pane fade" id="bill" role="tabpanel" aria-labelledby="bill-tab">
                            <div class="row">
                                ...
                                <div class="row">
                                    ...
                                    <select class="form-control select2 {{ $errors->has('term') ? 'is-invalid' : '' }}" name="term_id_inv" id="term_id_inv">
                                        @foreach($terms as $id => $entry)
        ...
        <div class="modal-body">
            <form method="" action="" enctype="multipart/form-data">
              @csrf
                ...
                <div class="form-group">
                    {{-- <textarea class="ckeditor form-control" name="term_editor" id="term_editor"></textarea> --}}
    ...
    $(document).ready(function(){
        $('#customer_id').select2({
            ...
            ajax: { // instead of writing the function to execute the request we use Select2's convenient helper
                url: "{{ route("frontend.get-customers") }}",
                dataType: 'json',
                quietMillis: 250,
        ...
        $('#product_id').select2({
            ...
            minimumInputLength: 1,
            ajax: { // instead of writing the function to execute the request we use Select2's convenient helper
                url: "{{ route("frontend.get-products") }}",
                dataType: 'json',
                quietMillis: 250,
                data: function (term, page) {
                    return {
        ...
        $("#product_save").click(function(){
            ...
            $.ajax({
                url: "{{ route('frontend.products-store')}}",
                type: "get",
                data: {
                        'item_type':item_type,
                        'name' : product_name,
        ...
        $('#categories').select2({
                ...
                minimumInputLength: 1,
                ajax: { // instead of writing the function to execute the request we use Select2's convenient helper
                    url: "{{ route("frontend.get-category") }}",
                    dataType: 'json',
                    quietMillis: 250,
                    data: function (term, page) {
                        return {
        ...
        $("#category_save").click(function(){
            ...
            $.ajax({
                url: "{{ route('frontend.category-store')}}",
                type: "get",
        ...
        $('#account_type_id').select2({
            ...
            minimumInputLength: 1,
            ajax: { // instead of writing the function to execute the request we use Select2's convenient helper
                url: "{{ route("frontend.get-account-types") }}",
                dataType: 'json',
                quietMillis: 250,
                data: function (term, page) {
                    return {
        ...
        $("#acc_type_save").click(function(){
            ...
            $.ajax({
                url: "{{ route('frontend.account-types-store')}}",
                type: "get",
        ...
        $('#account_name_id').select2({
            ...
            minimumInputLength: 1,
            ajax: { // instead of writing the function to execute the request we use Select2's convenient helper
                url: "{{ route("frontend.get-account-names") }}",
                dataType: 'json',
                quietMillis: 250,
                data: function (term, page) {
                    return {
        ...
        $("#acc_name_save").click(function(){
            ...
            $.ajax({
                url: "{{ route('frontend.account-names-store')}}",
                type: "get",
        ...
        $('#terms_and_condition_id').select2({
            ...
            minimumInputLength: 1,
            ajax: { // instead of writing the function to execute the request we use Select2's convenient helper
                url: "{{ route("frontend.get-termCondition") }}",
                dataType: 'json',
                quietMillis: 250,
                data: function (term, page) {
                    return {
        ...
        $("#term_condition_save").click(function(){
            ...
            // var term_editor = CKEDITOR.instances['term_editor'].getData();
            var term_editor = $("#term_editor").val();

            $.ajax({
                url: "{{ route('frontend.store-termCondition')}}",
                type: "get",
        ...
        $(document).on('change','.quantity',function(){
            ...
            var rate = parseInt($(this).parent().next('td').find('input').val(), 10);
            var amount = $(this).parent().next('td').next('td').find('input').val(quantity * rate);
            $(".amount_inv").each(function(){
                amount_total += parseInt($(this).val(), 10);
            })
            // var amount_inv = parseInt($(".amount_inv").val())
            $("#subtotal").text('₹'+amount_total);
            $("#total").text('₹'+ amount_total)
            $("#balance_due").text('₹'+ amount_total)
            $("#balance_due2").text('₹'+ amount_total)
        ...
        $(document).on('change','.tax_gst',function(){
            var old_gst = $(this).attr('before_data');
            ...
            $(this).attr('before_data',$(this).val());

            var amount_total = 0;

            var current_tax_gst = parseFloat($(this).val());

            var half_gst = parseFloat(current_tax_gst/2);
            var count = $(this).parent().prev('td').prev('td').prev('td').prev('td').prev('td').prev('td').find('select').attr('data-attr');
            var current_index = $(this).parent().parent().attr('data-index');

            var quantity = parseFloat($(this).parent().prev('td').prev('td').prev('td').find('input').val(), 2);
            var rate = parseFloat($(this).parent().prev('td').prev('td').find('input').val(), 2);
            if(isNaN(rate))
            {
                rate = 0;
            }
            var amount = $(this).parent().prev('td').find('input').val(quantity * rate);
            var flag = false;
            $(".amount_inv").each(function(){
        ...
        $("#customerSave").click(function(){
            ...
            var term_id_inv = $("#term_id_inv").val();
            var group = $("#group").val();

            $.ajax({
                url: "{{ route('frontend.store-customer')}}",
                type: "get",
                data: {
                    '_token':$('input[name="_token"]').val(),
                    'title': title,
