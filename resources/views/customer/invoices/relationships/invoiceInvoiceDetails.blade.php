    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class=" table table-bordered table-striped table-hover datatable datatable-invoiceInvoiceDetails">
                    <thead>
                        <tr>
                            <th width="10">
                            ...
                            <tr data-entry-id="{{ $invoiceDetail->id }}">
                                ...
                                <td>
                                    @can('invoice_detail_show')
                                        <a class="btn btn-xs btn-primary" href="{{ route('admin.invoice-details.show', $invoiceDetail->id) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan

                                    @can('invoice_detail_edit')
                                        <a class="btn btn-link text-theme" href="{{ route('admin.invoice-details.edit', $invoiceDetail->id) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan

                                    @can('invoice_detail_delete')
                                        <form action="{{ route('admin.invoice-details.destroy', $invoiceDetail->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-invoiceInvoiceDetails:not(.ajaxTable)').DataTable({ buttons: dtButtons })
