...
<body class="sidebar-mini layout-fixed" style="height: auto;">
    <div class="wrapper">
        <nav class="main-header navbar navbar-expand bg-white navbar-light border-bottom">
            ...
            @php $timeCount =  \Auth::user()->userAssignedTaskDetails()->where('end_time', null)->count(); @endphp 
            @if($timeCount > 0)
             Task in Progress &nbsp;&nbsp;
             <img class="recorder" width="10px" src="{{asset('img/record.gif')}}" alt="">
            @else
            Task in Progress &nbsp;&nbsp;
             <img class="recorder d-none" width="10px" src="{{asset('img/record.gif')}}" alt="">
            @endif   
            ...
            <ul class="navbar-nav ml-auto">
                <li class="nav-item dropdown notifications-menu">
                    <a href="#" class="nav-link" data-toggle="dropdown">
                        ...
                        @php $alertsCount = \Auth::user()->userUserAlerts()->where('read', false)->count(); @endphp
                            @if($alertsCount > 0)
            ...
            @php   
                $companies_departments =  App\Models\DepartmentHead::with('company')->where('head_of_department_id',auth()->user()->id)->get();
            @endphp
            @if(auth()->user()->id != 1)
            <ul class="navbar-nav ml-2">
            <li class="nav-item dropdown company-menu">
                <a href="#" class="nav-link" data-toggle="dropdown">
                    @php 
                    $value = isset($_COOKIE['selectedcomp']) ? $_COOKIE['selectedcomp']: 'All Companies';
                    $comp_id = isset($_COOKIE['comp_name']) ? $_COOKIE['comp_name']: 'not_value';
                    @endphp
                    <span id="check_comp_reload" comp_id={{$comp_id}}>{{$value}}</span>
                    <i class="bi bi-building-fill-gear"></i>
                </a>
                <div class="company-dropdown dropdown-menu dropdown-menu-lg dropdown-menu-right">  
                <h6 href="#">All Companies List</h6>
                   <div class="form-group">
                        <input class="form-control" id="liveSearch" type="text" placeholder="Serach Company name ">
                   </div>
                   <div class="company-ser-dropdown" id="comp_id">
                    <span class="comp_id" company_id="0" company_name="All Companies">All Companies</span>
                    <div class="hidden_comp" id="remove_comp">
                        @foreach ($companies_departments as $data)
                        @if(isset($data->company))
                            <span class="comp_id" company_id="{{$data->company->id}}" company_name="{{$data->company->company_name}}" >{{ ($data->company->company_name) }}</span>
                        @endif
                        @endforeach
                    </div>    
                   </div>
  ...
  $.fn.dataTable.ext.classes.sPageButton = '';
    ...
    //# sourceMappingURL=adminlte.min.js.map
     ...
     <script>
       @if(auth()->user()->id != 1)
        window.addEventListener( "pageshow", pageReload);
        function pageReload(){
           var comp_id =  $('#check_comp_reload').attr("comp_id");
            var x_comp_id = getCookie();
            if(comp_id != 'not_value'){
                if(comp_id != x_comp_id){
                   window.location.reload(); 
                }
            }
        }
        @endif
        $(document).on('click', '.comp_id', function() {
            var comp_id =  $(this).attr("company_id"); 
            var comp_name = $(this).attr("company_name");
            setCookie('comp_name',comp_id,'');
            setCookie('selectedcomp',comp_name,'');
            window.location.reload();
        });
        function getCookie(name) {
            ...
            var x_comp_id = comp_id[1];
            console.log(x_comp_id);
            }
            return x_comp_id;
        }
        ...
        $(document).ready( function(){
            var company_value = $("#check_comp_reload").attr('comp_id');
            if(company_value == 'not_value'){
                setCookie('comp_name','0','');
                setCookie('selectedcomp','All Companies','');
            }
            $('#liveSearch').keyup( function(){
                var input = $(this).val();
                if(input != "" && input.length>=3){
                    $.ajax({
                        url:"{{ route('admin.companies.search-companies') }}",
                        type: "get",
                        data: {
                            'input': input
                        },
                        success: function(data) {
                            ...
                            $.each(data, function(index, value) {
                                ...
                                var comp_id = 'comp_id';
                                var company_remove= 'company_remove';
                                $('#comp_id').empty();
                                $('#comp_id').append("<span class ='"+comp_id+"' id='"+company_remove+"'remove_id='"+index+"' company_id='"+index+"' company_name='"+value+"'>" + value + " </span> ");
                ...
                if(input.length == 0){
                    $.ajax({
                        url:"{{ route('admin.companies.search-isEmpty-companies') }}",
                        type: "get",
                        data: {
                            'input': input
                        },
                        success: function(data) {
                            ...
                            $.each(data, function(index, value) {
                                ...
                                $('.hidden_comp').hide();
                                $('.hidden_comp').empty();
                                $('#company_remove').remove();
                                $('#comp_id').append("<span class ='"+comp_id+"' company_id='"+value.company.id+"' company_name='"+value.company.company_name+"'>" + value.company.company_name + " </span> ");
