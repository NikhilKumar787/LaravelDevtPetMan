...
<head>
    ...
    <link href="{{ asset('css/customer/custom-customer.css') }}" rel="stylesheet" />
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.10.2/font/bootstrap-icons.css" integrity="sha384-b6lVK+yci+bfDmaY1u0zE8YYJt0TZxLEAFyYSLHId4xoVvsrQu3INevFKo+Xir8e" crossorigin="anonymous">
    <link rel="stylesheet" type="text/css" href="https://common.olemiss.edu/_js/sweet-alert/sweet-alert.css">
    @yield('styles')
</head>
...
<body class="sidebar-mini layout-fixed" style="height: auto;">
    <div class="wrapper">
        <nav class="main-header navbar navbar-expand bg-white navbar-light border-bottom">
           ...
           {{-- @php
                $timeCount =  \Auth::user()->userAssignedTaskDetails()->where('end_time', null)->count()
            @endphp
            @if($timeCount > 0)
             Task in Progress &nbsp;&nbsp;
            ...
            @endif
             --}}
            {{-- welcome message amd company name of that customer --}}
            @php
            ...
            <ul class="navbar-nav ml-auto">
                <li class="nav-item dropdown">
                    <span style="text-transform: capitalize; font-weight: bolder">Welcome to {{ $company->company_name}}({{$user->name}})</span>
                </li>
            </ul>
            ...
            <ul class="navbar-nav ml-auto">
                <li class="nav-item dropdown notifications-menu">
                    ...
                    <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right notification-dropdown">
                        <div class="d-flex justify-content-between mb-1 align-items-center notification-header-bg">
                            <h6 class="m-0">Notification</h6>
                            @php
                                $alertsCount = \Auth::user()->userUserAlerts()->where('read', false)->count()
                            @endphp
                            @if($alertsCount > 0)
                                <span class="">{{ $alertsCount }}</span>
                            @endif
                        </div>
                                ...
                                <div class="notification-box">
                                    <a href="{{ $alert->alert_link ? $alert->alert_link : "#" }}" rel="noopener noreferrer">
                                        @if($alert->pivot->read === 0) <strong> @endif
                                            @php
                                                $invoice =  App\Models\Invoice::where('id',$alert->model_id)->first();
                                                if($invoice != null){
                                            ...
                                            @endphp
                                             ...
                                             <div class="d-flex justify-content-between">
                                                 <h5>{{$invoice->invoice_prefix}}{{$invoice->invoice_no}}</h5>
